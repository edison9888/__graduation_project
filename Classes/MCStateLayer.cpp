...
MCStateLayer::init()
{
    if (CCLayer::init()) {
        ...
        int contentHeight = winSize.height - 90;
        int contentHeightWithoutBottom = contentHeight * 2 / 3;
        int offsetX = 180;
        int offsetY = contentHeightWithoutBottom / 8 + 4;
        int offsetYInc = 45;
        int valuePositionX;
        ...
        /* PP */
        label = CCLabelTTF::create("体　力　值", "Marker Felt", fontSize);
        addChild(label);
        label->setAnchorPoint(ccp(0, 1)); /* 左上角 */
        label->setPosition(ccp(offsetX, winSize.height - offsetYInc));
        offsetYInc += offsetY;
